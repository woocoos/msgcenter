...
import { CreateMsgSubscriberInput, MsgEvent } from '@/generated/msgsrv/graphql';
import { DrawerForm } from '@ant-design/pro-components';
import { createSub, delSub } from '@/services/msgsrv/type';
import { getMsgEventWithSubs } from '@/services/msgsrv/event';
...
export default (props: {
  ...
  const
    ...
    getRequest = async () => {
      setLoading(true);
      const result = await getMsgEventWithSubs(props.id);
      if (result?.id) {
    ...
    onFinish = async () => {
      ...
      if (info) {
        ...
        let oldKeys: string[] = [];
        if (subject === 'user') {
          oldKeys = info.subscriberUsers?.map(s => s.userID as string) || [];
        } else if (subject === 'exUser') {
          oldKeys = info.excludeSubscriberUsers?.map(s => s.userID as string) || [];
        } else if (subject === 'orgRole') {
          oldKeys = info.subscriberRoles?.map(s => `${s.orgRoleID}`) || [];
        }
        
        const addKeys = targetKeys.filter(key => !oldKeys.includes(key));
        const delKeys = oldKeys.filter(key => !targetKeys.includes(key));
        ...
        if (delKeys.length) {
          const ids = delKeys.map(key => {
            let sub;
            if (subject === 'user') {
              sub = info.subscriberUsers?.find(item => item.userID == key);
            } else if (subject === 'exUser') {
              sub = info.excludeSubscriberUsers?.find(item => item.userID == key);
            } else if (subject === 'orgRole') {
              sub = info.subscriberRoles?.find(item => `${item.orgRoleID}` == key);
            }
            return sub?.id || '';
          }).filter(id => !!id);
    ...
    >
      {
        loading ? <Skeleton /> : info ?
          <Space direction="vertical">
            <div>
              {t('msg_event')}：{info.msgType?.category}-{info.msgType?.name}-{info.name}
            </div>
            <div>
